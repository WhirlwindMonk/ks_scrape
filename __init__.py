...
def ksstatus(bot, trigger):
    ...
    for ks in f:
        ...
        name = name.strip()
        address = address.strip()
        if trigger.group(2).lower() in name.lower():
            tks = ks
            break
    f.close()
    if tks == None:
        return bot.say("Kickstarter project '" + trigger.group(2) + "' not found.")
    ks_data = get_data(tks)
    response = name + ": " + ks_data['pledged'] + " of " + ks_data['goal'] + " goal by " + ks_data['backer_count'] + " backers with " + ks_data['time_remaining'] + " left to go. Project is " + ks_data['percent'] + " funded. " + address
    return bot.say(response)
    
@module.commands('addks')
def addks(bot, trigger):
    ks_data = add_ks(trigger.group(2))
    if ks_data == None:
...
def remks(bot, trigger):
    response = rem_ks(trigger.group(2))
    if response == None:
